...
import {
  ...
  useAllBanners,
} from "@/src/features/banners/hooks/use-all-banners";
...
function AdvancedFiltersContent({ filters }: { filters: IGetPaginatedParams }) {
  ...
  const defaultValues: IGetPaginatedParams = {
    ...filters,
    size: filters.size ?? INITIAL_BANNERS_FILTERS.size,
  };
  ...
  const handleApplyFilters = (data: IGetPaginatedParams) => {
    setFiltersGlobal(BANNERS_COMPONENT_FILTER_KEY, {
      ...filters,
      ...data,
      page: 1,
    });
  ...
  const clearSearch = () => {
    ...
    form.reset(INITIAL_BANNERS_FILTERS);
    close();
  };
...
export function SearchFilters() {
  const { filters, onSearchChange } = useAllBanners();
