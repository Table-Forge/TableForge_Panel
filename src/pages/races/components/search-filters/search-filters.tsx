...
import {
  INITIAL_RACES_FILTERS,
  RACES_COMPONENT_FILTER_KEY,
  useAllRaces,
} from "@/src/features/races/hooks/use-all-races";
...
function AdvancedFiltersContent({ filters }: { filters: IGetPaginatedParams }) {
  ...
  const defaultValues: IGetPaginatedParams = {
    ...filters,
    size: filters.size ?? INITIAL_RACES_FILTERS.size,
  };
  ...
  const handleApplyFilters = (data: IGetPaginatedParams) => {
    setFiltersGlobal(RACES_COMPONENT_FILTER_KEY, {
      ...filters,
      ...data,
      page: 1,
    });
  ...
  const clearSearch = () => {
    ...
    form.reset(INITIAL_RACES_FILTERS);
    close();
  };
...
export function RacesSearchFilters() {
  const { filters, onSearchChange } = useAllRaces();
