...
import type { IInput } from "./input.intefaces";
  ...
  (
    ...
    return (
      <div className={`flex w-full flex-col gap-1 ${wrapperClassName ?? ""}`}>
        <div className={getInputClasses(error, isLoading, disabled)}>
          {isLoading ? (
            ...
            <input
              ...
              className={`${inputInnerClasses} ${uppercase ? "uppercase" : ""} ${className ?? ""}`}
            />
          )}
        </div>
