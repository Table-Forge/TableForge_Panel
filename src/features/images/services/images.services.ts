...
import { dataUrlToCompressedFile, dataUrlToFile, isImageDataUrl } from "@/src/utils/image";
...
export const ImageService = {
  ...
  create: async (payload: IImage): Promise<IImage> => {
    ...
    const file = payload.optimize !== false
      ? await dataUrlToCompressedFile(payload.content, payload.name)
      : dataUrlToFile(payload.content, payload.name);
