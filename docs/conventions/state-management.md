...
## What NOT to do
- **Don't use Redux or React Context for global state**: Zustand is our chosen global state tool. Neither provider in `src/context/` is a Context: `auth.tsx` only triggers hydration on mount, and `signalr.tsx` only opens the SignalR connection and writes incoming events straight into the TanStack Query cache — nothing consumes the connection object, so it does not need to be exposed. The one real Context in the codebase is component-local wiring for a compound component (`src/components/filters/filters.context.ts`), which is acceptable at that scope.
